...
Image-loop retry policy is separate from [zero-byte HTTP stream recovery](../transports/streaming-health.md#zero-byte-http-stream-recovery); the latter wraps only the native Chat or Responses first body leg.
