  ...
  let stepping = false;
      ...
      const closeCurrentMessage = (inferredPhase?: OcxMessagePhase) => {
        ...
        emit("response.output_text.done", {
          item_id: currentMsg.itemId, output_index: currentMsg.outputIndex,
          content_index: 0, text: messageText,
        });
        emit("response.content_part.done", {
          item_id: currentMsg.itemId, output_index: currentMsg.outputIndex,
          content_index: 0, part: { type: "output_text", text: messageText, annotations },
        });
        const item = {
      ...
      const closeCurrentRawReasoning = () => {
        ...
        emit("response.reasoning_text.done", {
          ...
          content_index: 0, text: rawText,
        });
        const item = {
          type: "reasoning", id: currentRawReasoning.itemId, summary: [] as never[],
      ...
      const closeCurrentToolCall = () => {
        ...
        rememberExtraContentForReplay(currentToolCall.callId, currentToolCall.providerMetadata, replayCacheScope);
        const item = currentToolCall.toolSearch
          ? {
            type: "tool_search_call", id: currentToolCall.itemId, call_id: currentToolCall.callId,
            execution: "client", arguments: parseArgsObj(currentToolCall.args), status: "completed",
          }
          : currentToolCall.freeform
          ? {
            type: "custom_tool_call", id: currentToolCall.itemId, call_id: currentToolCall.callId,
            name: currentToolCall.name, ...(currentToolCall.namespace ? { namespace: currentToolCall.namespace } : {}),
          ...
          : {
            type: "function_call", id: currentToolCall.itemId, call_id: currentToolCall.callId,
            name: currentToolCall.name, arguments: argsStr, status: "completed",
            ...(currentToolCall.namespace ? { namespace: currentToolCall.namespace } : {}),
      ...
      const failCurrentToolCall = () => {
        ...
        const item = currentToolCall.toolSearch
          ? {
            type: "tool_search_call", id: currentToolCall.itemId, call_id: currentToolCall.callId,
            execution: "client", arguments: parseArgsObj(currentToolCall.args), status: "incomplete",
          }
          : currentToolCall.freeform
          ? {
            type: "custom_tool_call", id: currentToolCall.itemId, call_id: currentToolCall.callId,
          ...
          : {
            type: "function_call", id: currentToolCall.itemId, call_id: currentToolCall.callId,
            name: currentToolCall.name, arguments: argsStr, status: "incomplete",
            ...(currentToolCall.namespace ? { namespace: currentToolCall.namespace } : {}),
            ...(rememberAndSerializeExtraContent(currentToolCall.callId, currentToolCall.providerMetadata, replayCacheScope).extra ?? {}),
          };
        emit("response.output_item.done", { output_index: currentToolCall.outputIndex, item });
      ...
      const closeCurrentWebSearch = (status: string, queries: string[], sources: { url: string; title?: string }[]) => {
        if (!currentWebSearch) return;
        const item = {
          type: "web_search_call", id: currentWebSearch.itemId, status, action: webSearchAction(queries),
      ...
      const returnIterator = () => {
        ...
        const finishReturn = () => { try { it.return?.()?.catch(() => {}); } catch { /* already closed */ } };
        if (!iteratorStarted) {
          iteratorStarted = true;
          try {
            it.next().then(finishReturn, () => {}).catch(() => {});
      ...
      const cancelUpstreamOnce = () => {
        ...
        try { onCancel?.(); } catch { /* cancel callbacks must not throw */ }
        returnIterator();
      };
      let handlingTranslatorOverflow = false;
      terminateForTranslatorOverflow = (_error: unknown) => {
        ...
        while (!terminated && !closed && emittedFrames === emittedAtStart) {
          ...
          if (options?.compaction) {
            if (event.type === "text_delta") {
              compaction = appendString(compaction, event.text, "retained_collectors");
              continue;
            }
            if (event.type !== "done" && event.type !== "incomplete" && event.type !== "error") continue;
          }
          // A thinking block ends when non-thinking content arrives. Do not close on heartbeat:
          // that would break long thinking output into multiple blocks.
          ...
          switch (event.type) {
            ...
            case "text_delta": {
              ...
              if (!currentMsg) {
                ...
                currentMsg = {
                  itemId, outputIndex, text: emptyChunks(), citationFilter: createCitationMarkerFilter(),
                  ...(event.phase ? { phase: event.phase } : {}),
                };
              }
              currentMsg.text = appendString(currentMsg.text, event.text, "retained_collectors");
            ...
            case "thinking_delta": {
              if (options?.hideThinkingSummary) {
                flushHiddenRawReasoning();
                rawReasoningForNextToolCall = "";
                hiddenThinking = appendString(hiddenThinking, event.thinking, "reasoning");
                break;
              }
              if (currentMsg) closeCurrentMessage("commentary");
              if (currentRawReasoning) closeCurrentRawReasoning();
